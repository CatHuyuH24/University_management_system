...
-- INSERT 4000 DÒNG SINH VIÊN VIÊN, RỒI DỰA VÀO ĐÓ TẠO TÀI KHOẢN, TÊN TK: MASV, MK MẶC ĐỊNH: 3 SỐ CUỐI DT + # + KHOA + ? + NAM SINH

-- CẦN SCRIPT TẠO TÀI KHOẢN SINH VIÊN -> NHƯNG ĐỂ XÁC ĐỊNH ĐÓ LÀ 1 "SINH VIÊN"
...
-- TẠO SINH VIÊN TEST THỬ
CREATE USER SV001 IDENTIFIED BY 123;
GRANT CREATE SESSION TO SV001;
GRANT SELECT ON SINHVIEN TO SV001;
GRANT UPDATE ON SINHVIEN TO SV001;
GRANT SELECT, INSERT, DELETE ON DANGKY TO SV001;


CREATE USER SV002 IDENTIFIED BY 123;
GRANT CREATE SESSION TO SV002;
GRANT SELECT ON SINHVIEN TO SV002;
GRANT UPDATE ON SINHVIEN TO SV002;
GRANT SELECT, INSERT, DELETE ON DANGKY TO SV002;

CREATE ROLE SINHVIEN;
GRANT CREATE SESSION TO SINHVIEN;
GRANT SELECT ON SINHVIEN TO SINHVIEN;
GRANT UPDATE ON SINHVIEN TO SINHVIEN;
GRANT SELECT, INSERT, DELETE ON DANGKY TO SINHVIEN;
GRANT SINHVIEN TO SV001;
ALTER USER SV001 DEFAULT ROLE SINHVIEN;

-- Tạo giảng viên test thử
CREATE USER NV003 IDENTIFIED BY 123;
GRANT CREATE SESSION TO NV003;
GRANT SELECT ON SINHVIEN TO NV003;
GRANT SELECT ON DANGKY TO NV003;

-- NHÂN VIÊN PHÒNG CTSV (KHÔNG XEM ĐƯỢC -> MỐT KIỂM TRA COI SỬA ĐƯỢC -> SỬA MÙ)
CREATE USER NV007 IDENTIFIED BY 123;
GRANT CREATE SESSION TO NV007;
GRANT SELECT, INSERT, UPDATE, DELETE ON SINHVIEN TO NV007;

-- NHAN VIEN PDT -> UPDATE TINHTRANG
CREATE USER NV008 IDENTIFIED BY 123;
GRANT CREATE SESSION TO NV008;
GRANT UPDATE(TINHTRANG) ON SINHVIEN TO NV008;
grant SELECT, INSERT, DELETE ON DANGKY to NV008;

-- NHÂN VIÊN PHÒNG KHẢO THÍ
CREATE USER NV009 IDENTIFIED BY 123;
GRANT CREATE SESSION TO NV009;
GRANT SELECT, UPDATE(DIEMTH, DIEMQT, DIEMCK, DIEMTK) ON DANGKY TO NV009;


SELECT * FROM MOMON;
select * from DANGKY D JOIN MOMON M ON D.MAMM = M.MAMM;
select * from nhanvien;
